            ...
            if ($getPeriod == 0 && !array_intersect($roleStatement, $getRole)) {
                ...
                if ($roleCount > 0) {
                    if ($firstRole == 'siswa') {
                        ...
                        #code before pendaftaran development issues
                        $inFinansial = WawancaraFinansial::where('user_id', $user->id)
                        ->whereNotNull('total_tunai')
                        ->exists();

                        $pembayaran = Pembayaran::where('user_id', $user->id)->get();

                        $inPembayaran = Pembayaran::where('user_id', $user->id)
                        ...
                        if ($inPendaftaran) {
                            if ($inDokumen) {
                                if($inPembayaran) {
                                    if($pembayaran[0]->isValid == "Menunggu" || $pembayaran[0]->isValid == "Valid"){
                                        if($inWawancara){
                                            if($inFinansial){
                                                return redirect()->route('hasilWawancaraFinansialShow', Auth::user()->id);
                                            }
                                            return redirect()->route('jadwalWawancaraSiswaShow', Auth::user()->id);
                                        }
                                    }
                                    elseif ($pembayaran[0]->isValid == "Tidak Valid") {
                                        // dd($pembayaran[0]->isValid); pembayaranTidakValid
                                        return redirect()->route('pembayaranTidakValid', Auth::user()->id);
                                    }
                                }
                                return redirect()->route('seleksiSiswaShow', Auth::user()->id);
                            }
