...
class UploadDokumenController extends Controller
{
    ...
    public function dokumenKelulusanSubmit(Request $request, $id){
        $wawancarafinansial = WawancaraFinansial::where('wawancara_finansial.user_id', $id)->get();
        $seragam = Seragam::where('seragam.user_id', $id)->get();
        ...
        if ($wawancarafinansial[0]->total_tunai != '0') {
            // dd($seragam[0]->seragam_senin == null || $seragam[0]->seragam_selasa == null || $seragam[0]->seragam_rabu == null || $seragam[0]->seragam_kamis == null || $seragam[0]->seragam_jumat == null);
            if ($seragam[0]->seragam_senin == null || $seragam[0]->seragam_selasa == null || $seragam[0]->seragam_rabu == null || $seragam[0]->seragam_kamis == null || $seragam[0]->seragam_jumat == null) {
                return redirect()->route('seleksiSiswaShow', Auth::user()->id);
            }
            return redirect()->route('seleksiSiswaShow', Auth::user()->id);
        }
    ...
    public function bergabungShow($id){
        ...
        $wawancarafinansial = WawancaraFinansial::where('wawancara_finansial.user_id', $id)->get();
        $seragam = Seragam::where('seragam.user_id', $id)->get();
        return view('pages.siswa.selesai.siswa-bergabung', compact('dokumen', 'pendaftaran', 'wawancarafinansial', 'seragam'));
    }

    public function pembayaranTidakValid($id){
        $pembayaran = Pembayaran::where('user_id', $id)->get();
        $pendaftaran = Pendaftaran::where('user_id', $id)->get();

        return view('pages.siswa.validasi.tidak-valid-pembayaran-siswa', compact('pembayaran', 'pendaftaran'));
    }

    public function pembayaranTidakValidSubmit(Request $request, $id){
        // dd($request->file_bukti_pembayaran_pendaftaran);
        $pathOld = Pendaftaran::where('user_id', $id)
        ->get();
        $pathOld = $pathOld[0]->file_bukti_pembayaran_pendaftaran;
        $validated = $request->validate([
            'file_bukti_pembayaran_pendaftaran' => 'required',
        ]);
        ...
        $new_path_file_bukti_pembayaran_pendaftaran = $request->file('file_bukti_pembayaran_pendaftaran')->store('uploads/'. Auth::user()->name , 'public');
